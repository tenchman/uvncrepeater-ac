...
/* In mode 2, require listed IDs ? (0=Allow all IDs, 1=Require that ID number
 * is in the list) */
...
/* if running as root, by default change to this user after all listen ports
 * have been bound */
 ...
 * 0=Allow all server addresses, 1=Only allow addresses/ranges defined
 * in ini file */
int requireListedServer = 0;

/* Mode 1 allowed servers (255.255.255.255 = Not allowed) */
struct in46_addr srvListAllow[SERVERS_LIST_SIZE];

/* Mode 1 denied servers (255.255.255.255 = Not denied) */
struct in46_addr srvListDeny[SERVERS_LIST_SIZE];
